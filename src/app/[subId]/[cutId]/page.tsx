...
const AppointmentBarberPage = ({ params }: { params: { cutId: string, subId: string } }) => {
  ...
  useEffect(() => {
    ...
    if (!savedEmail) {
      setShowPopup(true); // Mostrar pop-up se o e-mail não estiver salvo
    }
  ...
  const handleCreateAppointment = async (time: string) => {
    if (selectedBarber && selectedDate) {
      const localDateTime = new Date(`${format(selectedDate, "yyyy-MM-dd")}T${time}:00`);
      ...
      try {
        await clientServices.createAppointmentWithSubId({
          ...
          clientId: email, // Usar o e-mail salvo no estado ou no LocalStorage
          clientName: name,
        });
      } catch (error) {
        // Tratar erro
  ...
  // Salvar o nome e e-mail no LocalStorage e fechar o pop-up
  const handleSaveUserDetails = () => {
    if (email && name) {
      localStorage.setItem("userEmail", email);
      localStorage.setItem("userName", name);
      setShowPopup(false); // Fechar o pop-up após salvar
    }
  };
  ...
  return (
    <>
                ...
                {currentWeek.map((day) => (
                  <div
                    ...
                    className={`border p-4 rounded flex flex-col items-center justify-center h-14 ${
                      selectedDate?.toString() === day.toString() ? "bg-blue-500 text-white" : ""
                    }`}
      ...
      {/* Popup para coletar nome e e-mail */}
      <Dialog open={showPopup}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Insira seu nome e e-mail</DialogTitle>
          </DialogHeader>
          <div className="mt-4">
            <Input
              placeholder="Nome"
